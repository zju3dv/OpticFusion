...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        self.residual = models.make(self.config.residual.name, self.config.residual)
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.residual, epoch, global_step)
        if self.config.learned_background:
    ...
    def forward_(self, rays):
        ...
        residual_rgb = self.residual(feature, t_dirs, normal)
        ...
        opacity = accumulate_along_rays(weights, ray_indices, values=None, n_rays=n_rays)
        depth = accumulate_along_rays(weights, ray_indices, values=midpoints, n_rays=n_rays)
        comp_rgb = accumulate_along_rays(weights, ray_indices, values=rgb+residual_rgb, n_rays=n_rays)
        comp_residual = accumulate_along_rays(weights, ray_indices, values=residual_rgb, n_rays=n_rays)
